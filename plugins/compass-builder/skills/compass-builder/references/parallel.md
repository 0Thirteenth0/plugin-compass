...
Workers may commit only on their assigned `cb/<run-id>/<story-id>` branch. They never write shared controller state or integration Git and must not launch child workers, agents, or nested multi-agent sessions. The controller waits at a wave barrier, independently verifies the complete wave, and then integrates verified branches one at a time in immutable plan order.
